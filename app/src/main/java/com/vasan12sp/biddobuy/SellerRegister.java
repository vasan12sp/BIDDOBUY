    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        registerButton.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                registerSeller(name, phone, email, address, pincode, password);
            }
        });
    }

    private void registerSeller(String name, String phone, String email, String address, String pincode, String password) {
        ...
        ref.child(phone).addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                if (snapshot.exists()) {
                    Toast.makeText(SellerRegister.this, "Seller with this phone number already exists", Toast.LENGTH_SHORT).show();
                } else {
                    // Seller doesn't exist, proceed with registration
                    Sellers seller = new Sellers(name, phone, email, address, pincode, password);
                    ref.child(phone).setValue(seller)
                            .addOnCompleteListener(new OnCompleteListener<Void>() {
                                @Override
                                public void onComplete(@NonNull Task<Void> task) {
                                    if (task.isSuccessful()) {
                                        Toast.makeText(SellerRegister.this, "Successfully registered", Toast.LENGTH_SHORT).show();
                                        startActivity(new Intent(SellerRegister.this, MainActivity.class));
                                        finish();
                                    } else {
